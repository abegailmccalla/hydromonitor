...
class DB:
    ...
    def getAllInRange(self,start, end):
        ...
        try:
            ...
            remotedb 	= self.remoteMongo('mongodb://%s:%s@%s:%s' % (self.username, self.password,self.server,self.port), tls=self.tls)
            result      = list(remotedb.ELET2415.climo.find({'$and':[{'timestamp':{'$gte':start}}, {'timestamp':{'$lte':end}}]}, {'_id': 0}).sort('timestamp', 1)) #Add your query here        
    ...
    def humidityMMAR(self,start, end):
        ...
        try:
            ...
            remotedb 	= self.remoteMongo('mongodb://%s:%s@%s:%s' % (self.username, self.password,self.server,self.port), tls=self.tls)
            result      = list(remotedb.ELET2415.climo.aggregate([{'$match':{'$and':[{'timestamp':{'$gte':start}}, {'timestamp':{'$lte':end}}]}}, {'$group':{'_id': '$humidity','humidity': {'$push': '$$ROOT.humidity'}}}, {'$project': {'_id': 0, 'max': {'$max': '$humidity'}, 'min': {'$min': '$humidity'}, 'avg': {'$avg': '$humidity'}, 'range': {'$subtract': [{'$max': '$humidity'}, {'$min': '$humidity'}]}}}])) #Add your Aggregation pipeline here in this function
    ...
    def temperatureMMAR(self,start, end):
        ...
        try:
            ...
            remotedb 	= self.remoteMongo('mongodb://%s:%s@%s:%s' % (self.username, self.password,self.server,self.port), tls=self.tls)
            result      = list(remotedb.ELET2415.climo.aggregate([{'$match':{'$and':[{'timestamp':{'$gte':start}}, {'timestamp':{'$lte':end}}]}}, {'$group':{'_id': '$temperature','temperature': {'$push': '$$ROOT.temperature'}}}, {'$project': {'_id': 0, 'max': {'$max': '$temperature'}, 'min': {'$min': '$temperature'}, 'avg': {'$avg': '$temperature'}, 'range': {'$subtract': [{'$max': '$temperature'}, {'$min': '$temperature'}]}}}])) #Add your Aggregation pipeline here in this function                                                       #$cond:{ if:{$and:[{timestamp:{$gte:start}}, {timestamp:{$lte:end}}]}, then:{$max:temperature, $min:temperature, $avg:temperature , $range:[$max:temperature, $min:temperature]}})     
    ...
    def frequencyDistro(self,variable,start, end):
        ...
        try:
            remotedb 	= self.remoteMongo('mongodb://%s:%s@%s:%s' % (self.username, self.password,self.server,self.port), tls=self.tls)
            result      = list(remotedb.ELET2415.climo.aggregate([{'$match': {'timestamp': { '$gte': start, '$lte': end}}}, {'$bucket': {'groupBy': f"${variable}",'boundaries': list(range(101)), 'default': 'outliers','output': {'count': { '$sum': 1 }}}}]))
